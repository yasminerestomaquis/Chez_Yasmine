...
| 2026-09-06 | L'isolation multi-tenant RLS (Phase 16) a été vérifiée par une vraie requête Postgres (`set local role authenticated` + `request.jwt.claims` simulant deux utilisateurs de deux organisations, lecture et écriture croisées testées), pas seulement par les advisors statiques déjà validés en Phase 3/5 | Les advisors analysent la définition des policies, pas leur effet réel sur une requête ; un test avec une identité simulée est la seule façon de prouver empiriquement l'isolation plutôt que de la déduire de la lecture du SQL. Toutes les données de test ont été supprimées après coup (base revérifiée à zéro ligne) |
| 2026-09-06 | Les deux scénarios E2E du prompt maître §39 n'ont pas été exécutés en Phase 16 | Un E2E réel demande l'API NestJS effectivement démarrée contre `DATABASE_URL`, toujours indisponible depuis la Phase 4 ; les fabriquer sans backend réel aurait violé la règle de non-fabrication (prompt maître §48) — chaque étape reste individuellement testée dans sa phase d'origine |
| 2026-09-06 | Le job `deploy` du pipeline de production (Phase 17) est structurellement en place (`needs: [api, web]`, branche `production` dédiée) mais désactivé (`if: false`) | Aucun hébergeur n'a été choisi ; un déploiement fabriqué qui ne déploierait nulle part aurait été un mensonge vert dans l'interface GitHub Actions plutôt qu'une fonctionnalité livrée |
| 2026-09-06 | `JWT_SECRET`, `SUPABASE_SERVICE_ROLE_KEY` et `STORAGE_BUCKET` restent listés dans `.env.example` (conformité à la liste du prompt maître §37) bien qu'aucun ne soit lu par le code, chacun annoté avec la décision qui l'a rendu inutile | Les supprimer aurait pu passer pour un oubli face à la liste explicite du prompt maître ; les laisser sans explication aurait laissé croire qu'ils sont requis. `API_URL`, en revanche, était censé être utilisé mais était figé en dur côté Flutter — corrigé (`String.fromEnvironment`) plutôt qu'annoté, puisque lui a un vrai usage à couvrir (Phase 17) |
| 2026-09-06 | `gh` reconnecté au compte `yasminerestomaquis` via device flow (deux passages : d'abord scope `repo` seul, puis `gh auth refresh -s workflow` après un premier push rejeté faute du scope `workflow`), et `git` configuré pour utiliser ses identifiants (`gh auth setup-git`) | Le compte précédemment authentifié (`Autocad-Qgis`) n'avait qu'un accès en lecture au dépôt cible ; GitHub refuse par ailleurs qu'une App OAuth modifie `.github/workflows/*` sans le scope `workflow` explicitement accordé, découvert au premier essai de push plutôt que supposé |
| 2026-09-06 | Le mot de passe Postgres réel a été fourni par l'utilisateur et placé dans `.env` (jamais committé) ; utilisé pour démarrer l'API NestJS en local pour la première fois et vérifier `SupabaseJwtGuard`/`PermissionsGuard`/le trigger `handle_new_user` avec un vrai jeton (inscription + confirmation directe en SQL, faute d'accès à la boîte mail de test) | Referme le blocage `DATABASE_URL` documenté depuis la Phase 4 ; les données de test (compte, organisation, établissement) ont été supprimées immédiatement après, base revérifiée à zéro ligne — même discipline de nettoyage que le test RLS de la Phase 16 |
| 2026-09-06 | Le premier run réel du pipeline CI a révélé deux bugs invisibles en local : un `package-lock.json` incohérent (`tsconfck` avait `typescript` dédupliqué vers une version invalide au lieu de son propre `^5.0.0` nécessaire) et l'absence d'un `npx prisma generate` après `npm ci` (le client Prisma généré n'a pas de hook `postinstall`) | Ni l'un ni l'autre n'était détectable en local, où `node_modules` avait été installé une seule fois au tout début du projet (Phase 2) et jamais réinstallé depuis — `npm run build`/`test` réutilisaient silencieusement un état qui aurait échoué sur une machine neuve. Corrigés et revérifiés par un `npm ci` propre de bout en bout avant de repousser |
