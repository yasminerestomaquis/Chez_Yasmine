...
- `SyncQueueService` ([lib/sync/sync_queue_service.dart](../../apps/web/flutter/lib/sync/sync_queue_service.dart)) : file locale persistée via `shared_preferences`, une par établissement. `enqueue()` ajoute une opération ; `syncAll()` envoie tout le lot en attente — appelé aussi bien par la synchronisation automatique au retour de connexion que par le clic manuel sur « Synchroniser », qui déclenchent tous deux exactement le même code. **Testé en conditions réelles** (persistance locale, isolation entre établissements, ordre conservé — 4 tests, sans mock réseau).
  - **Découpage en lots de 100 (correctif 2026-09-13)** : le serveur plafonne un lot à 100 opérations (`SyncBatchDto`). Envoyer toute la file en une seule requête au-delà de cette taille l'aurait fait rejeter en bloc (400), bloquant définitivement toute progression — exactement la perte de données par blocage que le bouton de synchronisation ne doit jamais produire. `syncAll()` envoie désormais la file par lots d'au plus 100, chaque lot n'étant retiré de la file persistée qu'une fois sa propre réponse reçue : l'échec (ou la coupure) d'un lot ne fait jamais perdre un lot précédent déjà synchronisé, et laisse le reste en file pour le prochain essai. Renvoyer un lot déjà traité lors d'un essai ultérieur reste sans danger — toute la chaîne est idempotente sur l'id de chaque opération.
