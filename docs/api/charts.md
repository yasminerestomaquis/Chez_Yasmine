...
- **Total hebdomadaire affiché en tête de graphique** (`WeekTotalBadge`, `weekly_bar_chart.dart`, décision actée 2026-09-17) : en haut à droite des cartes "journalières totales" et "journalières totales par catégorie" (Recettes, Bénéfices, Dépenses) — calcul purement client (`WeeklyChart.total`, somme de tous les points de toutes les séries déjà reçues), aucune route dédiée nécessaire.
