...
Bouton d'export dans l'AppBar du module Stock (`stock_page.dart`, haut à droite, icône PDF), **réservé au rôle Super Administrateur** (`StockPage._isSuperAdmin`, comparaison par nom de rôle — même limitation que `_isServeur` dans le même fichier, `GET /auth/me` n'exposant pas de code de permission dédié au client) : un listing, une ligne par produit (jamais deux lignes pour le même), agrégé sur ses seuls lots FIFO de statut `'actif'` — même critère que « Lots actifs » ci-dessus. `ChartsService.activeStockListing` fait la somme, sur ces lots, de `receivedQuantity`/`lossQuantity`/`remainingQuantity` ; produits sans aucun lot actif absents du résultat (jamais approvisionnés, ou entièrement épuisés).
...
- UI Flutter (`lib/charts/`, `lib/stock/`) : `flutter analyze` ✅ (0 issue), `flutter test` ✅. `WeeklyChart.total` (2 tests, `chart_models_test.dart`) : somme de tous les points de toutes les séries, zéro pour une liste de séries vide. `graphiques_page_test.dart` (2026-09-17) : le filtre catégorie Dépenses est bien un `FilterChip` multi-sélection avec un chip "Toutes". `flutter build web` ✅.
