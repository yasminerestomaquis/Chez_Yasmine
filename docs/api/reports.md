...
Le prompt maître (§34) demande PDF/Excel/CSV. Seul le **CSV** est livré (`GET .../summary.csv`, un indicateur par ligne) — sans dépendance supplémentaire, entièrement testable en pur TypeScript. PDF et Excel sont délibérément **reportés** : les deux demandent une vraie bibliothèque de rendu, non encore choisie ni testée, et cette application n'a toujours pas de round-trip HTTP vérifié en conditions réelles (voir plus bas) — les ajouter maintenant aurait été une fonctionnalité non vérifiable plutôt qu'un vrai livrable.
...
- `ReportsService.summary` : 4 tests (Prisma mocké) — exclusion des ventes annulées et filtrage établissement/période, calcul chiffre d'affaires/marge/bénéfice net, classement des produits les plus vendus, agrégation par serveur.
- `ReportsService.summaryCsv` : 1 test — en-tête et au moins un indicateur présents.
- UI Flutter (`lib/reports/`) : sélecteur de période, export CSV affiché dans un dialogue (copiable), et — comme détaillé ci-dessus — un test qui exerce spécifiquement le changement de période jusqu'à son rechargement complet. `flutter analyze`/`test`/`build web` ✅.
