...
Côté Flutter, un bouton (icône liste) en haut à droite de la Caisse et de l'écran Addition ouvre le listing des ventes du jour (`GET .../sales?day=`, route déjà existante — aucun nouvel endpoint de lecture), avec une action de modification par ligne (quantité) et par paiement (mode) — voir `lib/pos/sold_items_page.dart`, partagé entre les deux écrans.
