...
- Le groupe lui-même (`_StockValueBox`) a quitté la rangée de compteurs — il vit dans son propre encadré, avec son propre filtre Catégorie à sélection multiple (`_valueCategoryIds`, indépendant du filtre de la liste de produits) et trois vignettes calculées sur les catégories choisies (tout le catalogue sans sélection) : **Prix d'achat**/**Prix de vente** (`stockValueTotals`, `lib/stock/stock_value.dart`) et **Bouteilles en stock** (« Nombre total de bouteilles en stock » à l'introduction, renommée le même jour — `stockBottleCount`, restreinte aux catégories `hasCasePricing` — Bières, Vins, Sucreries — même filtrée sur une autre catégorie).

## Ajout (2026-09-22, suite) — 4ᵉ vignette de la rangée du haut, 3 vignettes du groupe en ligne, bouteilles filtrables
...
- Les 3 vignettes du groupe (Prix d'achat, Prix de vente, Bouteilles en stock) sont désormais sur une seule ligne horizontale plutôt que 2+1.
