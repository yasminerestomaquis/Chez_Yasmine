...
- Le groupe lui-même (`_StockValueBox`) a quitté la rangée de compteurs — il vit dans son propre encadré, avec son propre filtre Catégorie à sélection multiple (`_valueCategoryIds`, indépendant du filtre de la liste de produits) et trois vignettes calculées sur les catégories choisies (tout le catalogue sans sélection) : **Prix d'achat**/**Prix de vente** (`stockValueTotals`, `lib/stock/stock_value.dart`) et **Nombre total de bouteilles en stock** (`stockBottleCount`, restreinte aux catégories `hasCasePricing` — Bières, Vins, Sucreries — même filtrée sur une autre catégorie).
