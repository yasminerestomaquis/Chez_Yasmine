...
- **Bug corrigé : échec de l'upload d'une photo produit dont le nom de fichier contient un caractère hors Latin-1** (`apps/web/flutter/lib/api/api_client.dart`) — `Cannot convert argument to a ByteString because the character at index N has a value of M which is greater than 255`. Le nom de fichier original (caméra/galerie/sélecteur de fichiers, fourni par l'OS/le navigateur) était transmis tel quel dans la requête multipart ; sur Flutter Web, l'API navigateur sous-jacente exige un nom Latin-1. Corrigé par `ApiClient._safeFilename()` : ne conserve que l'extension du fichier d'origine et reconstruit un nom `photo.<ext>` toujours sûr — sans conséquence côté serveur, qui renomme de toute façon le fichier en UUID.
