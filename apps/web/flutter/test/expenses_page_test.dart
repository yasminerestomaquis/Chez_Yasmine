...
void main() {
  ...
  testWidgets('offers the predefined expense categories plus a free-text "Autre" option', (tester) async {
    await openAddDialog(tester);
  ...
  testWidgets('selecting "Autre" reveals a free-text field for the category', (tester) async {
    await openAddDialog(tester);
    ...
    expect(find.widgetWithText(TextField, 'Préciser la nature'), findsOneWidget);
  });

  testWidgets('defaults the periodicity selector to Ponctuelle, with Récurrente selectable', (tester) async {
    await openAddDialog(tester);
